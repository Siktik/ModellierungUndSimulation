...
public class SimulationManager {
    ...
    /**
     * Same method as above, but for Multi-Queue. Instead of adding the event to a single queue it is added to
     * the multiple queues in a strict order, so every queue has gotten exactly 1/3 of events (or at most one event more
     * than the other queues).
     */
    private static void checkForNewArrivingEventsMQ(){
        if(!allArrivalEvents.isEmpty()) {
            if(allArrivalEvents.get(0).getTimestampOfExecution()< TimeManager.getElapsedTimeInMilliSeconds()){
                // Next Queue to receive an event is determined using modulo.
                // This way, target queue order is always 1,2,3,1,2,3,1,...
